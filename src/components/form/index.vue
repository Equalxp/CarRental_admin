<template>
  <el-form ref="form" :model="formData" :label-width="labelWidth">
    <el-form-item v-for="item in formItem" :key="item.prop" :label="item.label" :prop="item.prop" :rules="item.rules">
      ...
      <!-- InputNumber -->
      <el-input-number v-if="item.type === 'InputNumber'" v-model="formData[item.prop]" controls-position="right" :min="item.min || 0" :max="item.max || 100000"></el-input-number>
      <!-- Select -->
      ...
      <template v-if="item.type === 'Upload'">
        <!--  -->
        <Upload :requestFlag="item.requestFlag" :imgUrl="formData[item.prop]" :value.sync="formData[item.prop]"></Upload>
      </template>
    </el-form-item>
    <!-- 按钮 -->
    <el-form-item>
